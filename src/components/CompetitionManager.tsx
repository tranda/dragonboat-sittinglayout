    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-[var(--bg-overlay)] pt-6">
      <div className="bg-[var(--bg-surface)] rounded-xl shadow-2xl w-full max-w-md mx-4 max-h-[90dvh] flex flex-col">
        <div className="flex items-center justify-between p-4 border-b">
          <h2 className="text-lg font-bold text-[var(--text-primary)]">Competitions & Teams</h2>
          <button onClick={onClose} className="text-[var(--text-muted)] hover:text-[var(--text-secondary)] text-xl px-1">&times;</button>
        </div>

        {/* Tabs */}
        <div className="flex border-b">
          <button onClick={() => setTab('competitions')} className={`flex-1 py-2 text-sm font-semibold ${tab === 'competitions' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-[var(--text-muted)]'}`}>
            Competitions
          </button>
          <button onClick={() => setTab('teams')} className={`flex-1 py-2 text-sm font-semibold ${tab === 'teams' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-[var(--text-muted)]'}`}>
            Teams
          </button>
        </div>
              ...
              {showAddTeam ? (
                <div className="border-2 border-dashed border-green-300 rounded-lg p-3 space-y-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={clearTeamForm} className="px-3 py-1.5 text-xs bg-[var(--bg-surface-alt)] rounded-lg">Cancel</button>
                  </div>
                </div>
              ) : (
