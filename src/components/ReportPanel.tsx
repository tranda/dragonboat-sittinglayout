...
export function ReportPanel({ athletes, races, layouts, config, onClose }: Props) {
  ...
  // Role counts per athlete
  const rows = useMemo(() => {
    const counts = new Map<number, RoleCounts>();
    const add = (id: number | null, role: keyof RoleCounts) => {
      if (id === null) return;
    ...
    for (const race of filteredRaces) {
      ...
      if (!layout) continue;
      layout.left.forEach(id => add(id, 'paddle'));
      layout.right.forEach(id => add(id, 'paddle'));
      add(layout.drummer, 'drummer');
      add(layout.helm, 'helm');
      layout.reserves.forEach(id => add(id, 'reserve'));
    }
    return athletes
